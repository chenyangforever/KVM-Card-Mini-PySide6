...
def load_icon(name, dark_override=None) -> QIcon:
    if dark_override is not None:
        ...
        judge = dark_theme
    if judge:
        return QIcon(f"{PATH}/resources/icons/24_dark/{name}.png")
    else:
        return QIcon(f"{PATH}/resources/icons/24_light/{name}.png")
...
def load_pixmap(name, dark_override=None) -> QPixmap:
    if dark_override is not None:
        ...
        judge = dark_theme
    if judge:
        return QPixmap(f"{PATH}/resources/icons/24_dark/{name}.png")
    else:
        return QPixmap(f"{PATH}/resources/icons/24_light/{name}.png")
...
class MyMainWindow(QMainWindow, main_ui.Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        try:
            with open(
                os.path.join(PATH, "resources", "data", "keyboard_hid2code.yaml"), "r"
            ) as load_f:
                self.keyboard_hid2code = yaml.safe_load(load_f)
            with open(
                os.path.join(PATH, "resources", "data", "keyboard_scancode2hid.yml"),
                "r",
            ) as load_f:
                self.keyboard_scancode2hid = yaml.safe_load(load_f)
            with open(
                os.path.join(PATH, "resources", "data", "keyboard.yaml"), "r"
            ) as load_f:
        ...
        # 窗口图标
        self.setWindowIcon(QIcon(f"{PATH}/resources/icons/icon.ico"))
...
def main():
    ...
    if translation:
        if translator.load(os.path.join(PATH, "resources", "trans_cn.qm")):
